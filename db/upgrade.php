...
function xmldb_theme_pslearning_upgrade($oldversion) {
    ...
    if ($oldversion < 2013041200) {
        // Migrate logo URL.
        $logo = get_config('theme_pslearning', 'logo');
        if ($logo === '') {
            ...
            if ($content = download_file_content($logo)) {
                ...
                $fs = get_file_storage();
                $record = array(
                    'contextid' => context_system::instance()->id, 'component' => 'theme_pslearning',
                    'filearea' => 'logo', 'itemid'=>0, 'filepath'=>'/', 'filename'=>$filename);
                $fs->create_file_from_string($record, $content);
                set_config('logo', '/'.$filename, 'theme_pslearning');
                unset($content);

            } else {
                unset_config('theme_pslearning', 'logo');
            }
        } else {
        ...
        upgrade_plugin_savepoint(true, 2013041200, 'theme', 'pslearning');
    }
